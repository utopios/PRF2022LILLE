...
using TpListContactClass.Classes;


namespace TpListContactIHM.Classes
{
    internal class IHM
    {
        Annuaire annuaire;
        ...
        public void Init()
        {
            annuaire.Contacts.Add(new("Jean-michel", "Apeupré", new DateTime(1968, 09, 21), new Address(5, "rue du petit pont", 59780, "Willems", "France"), "+33 6 74 85 96 32", "jm@voila.net"));
            annuaire.Contacts.Add(new("Eléonore", "Marolex", new DateTime(1986, 06, 18), new Address(48, "rue du bling bling", 75017, "Paris", "France"), "+33 2 25 74 96 32", "elenore.marolex@swatch.com"));
            annuaire.Contacts.Add(new("Jeanne", "Dark", new DateTime(2000, 01, 01), new Address(7, "rue du bucher", 59000, "Lille", "France"), "+33 6 74 85 96 32", "jeanne.dark@gmail.com"));
        }
        ...
        public void AddContactAction()
        {
            ...
            try
            {
                int day, month, year, number, postalcode, nb1 = annuaire.Contacts.Count(), nb2;
                ...
                annuaire.Contacts.Add(c);
                // Vérification du nombre d'objet dans la liste
                nb2 = annuaire.Contacts.Count();
        ...
        public void UpdateContactAction()
        {
            ...
            try
            {
                ...
                // Itération de la liste de contact
                for (int i = 0; i < annuaire.Contacts.Count; i++)
                {
                    // Si l'id du contact présent à l'index i correspond à l'id saisi
                    if (annuaire.Contacts[i] == c)
                    {
                        // Updates des Props
                        annuaire.Contacts[i].Firstname = firstname;
                        annuaire.Contacts[i].Lastname = lastname;
                        annuaire.Contacts[i].DateOfBirth = dateOfBirth;
                        annuaire.Contacts[i].ContactAddress = contactAdress;
                        annuaire.Contacts[i].PhoneNumber = phone;
                        annuaire.Contacts[i].Email = email;
                        updated = true;
                        OnGreen("\nContact modifié...\n");
                    }
                }
                // Si le booléen updated est à false
                if (!updated)
        ...
        public void DeleteContactAction()
        {
            ...
            if (c != null)
            {
                int nb1 = annuaire.Contacts.Count();
                annuaire.Contacts.Remove(c);
                int nb2 = annuaire.Contacts.Count();
                if (nb1 - nb2 == 1)
        ...
        public (bool, Contact) GetContact(int id)
        {
            ...
            {
                foreach (Contact c in annuaire.Contacts)
                {
                    if (c.Id == id)
                    {
                        contact = c;
